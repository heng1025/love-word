...
let make = (~loading, ~errText, ~results, ~className) => {
  ...
  React.useEffect1(() => {
    setTransList(._ => Js.Array2.map(results, v => add(v, false)))
    None
  }, [results])

  let onPlay = (source: string) => {
    setTransList(.p => Js.Array2.map(p, v => add(v, v["dst"] === source)))

    audioEl.current
    ->Js.Nullable.toOption
    ->Belt.Option.forEach(audio => {
      audio->Element.createAudioSrc(Baidu.textToSpeech(source))
      audio->Element.play
    })
  }

  <div className>
    <audio ref={ReactDOM.Ref.domRef(audioEl)} controls=true className="w-full h-8 mb-1" />
    {switch loading {
      ...
      | false =>
        React.array(
          Js.Array2.mapi(transList, (result, idx) => {
            ...
            let animate200ms = checked ? "animate-fadeInOut-200ms" : ""
            let animate400ms = checked ? "animate-fadeInOut-400ms" : ""
